...
namespace SyncChanges.Core
{
    ...
    public class Synchronizer
    {
        ...
        private void Replicate(DatabaseInfo source, DatabaseInfo destinations, IList<TableInfo> tables)
        {
                    ...
                    //using (var transaction = db.GetTransaction(System.Data.IsolationLevel.ReadUncommitted))
                    {
                        try
                        {
                            ...
                            for (int i = 0; i < changes.Count; i++)
                            {
                                ...
                                PerformChange(/*db, */ change, destination);
                            ...
                            if (!DryRun)
                            {
                                SetSyncVersion(/*db, */changeInfo.Version, destination);
        ...
        private void SetSyncVersion(/*Database db, */long currentVersion, DatabaseInfo destination)
        {
            if (!DryRun)
            {
        ...
        private ChangeInfo RetrieveChanges(DatabaseInfo source, DatabaseInfo destinations, IList<TableInfo> tables)
        {
            var destinationVersion = GetDestinationVersion(destinations);
        ...
        private void PerformChange(/* Database db, */ Change change, DatabaseInfo destination)
        {
        ...
        private long GetDestinationVersion(DatabaseInfo destination)
        {
            long version = 0;
            ...
            // get destination version from file | already sync
            string filePath = string.Format("{0}\\_version.txt", destination.ConnectionString);

            // write version in file
            string text = System.IO.File.ReadAllText(filePath);

            long.TryParse(text, out version);
